...
const Home = () => {
  ...
  const filteredPlaces = places.filter(
    ({ name, city, type }) =>
      name.trim().toLowerCase().includes(info.place.trim().toLowerCase()) &&
      (city.includes(info.city) || info.city === "All") &&
      (type.includes(info.type) || info.type === "All")
  );
  ...
  return (
    <div className="bg-dark-subtle body">
      ...
      <p className="text-center text-primary">
        (Şu an için{" "}
        {[...new Set(places.map(({ city }) => city))].map((city) => city + " ")}{" "}
        hizmetimiz vardır.) (Rezervasyon için kutulara çift tıklatın.)
      </p>
      <Container>
        <MyForm
          cities={[...new Set(places.map(({ city }) => city))]}
          types={[...new Set(places.map(({ type }) => type))]}
